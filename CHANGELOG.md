...
- **Verificado en el navegador (demo, como administración):** orden de ingreso, orden de egreso (orden de áreas y descuento: 60.000 - 12.000 = 48.000), presupuesto (neto 40.336, IVA 7.664), factura prefillada (48.000, igual al egreso), aplicar, autorizar y quitar el descuento. Sin probar con rol asesor (límite de 15% y envío de solicitud).
...
- **Bandeja de salida en la base, no una llamada frágil:** `citas.clickup_pendiente` nace en `true` y un trigger (`trg_citas_pendiente_clickup`) lo vuelve a poner en `true` cuando cambia algo que ClickUp muestra (estado, fecha, hora, duración, descripción, isla, cliente, vehículo, servicio). Las citas anteriores a la migración quedan en `false` y no se envían solas. La sincronización solo toca columnas `clickup_*`, así que no reactiva el trigger. Fallos: `clickup_intentos` (a los 5 se deja de reintentar sola) y detalle en `integraciones_clickup_errores.cita_id`.
- **Código compartido** (`_shared/clickupCitas.ts`): lo usan la función nueva `clickup-agendar-cita` (v1, `verify_jwt` true; procesa las pendientes de la empresa de quien llama, sin recibir ids: no puede tocar tarjetas de otra empresa) y el bot de WhatsApp (v4), que sincroniza después de confirmarle al cliente para no demorarlo.
- **Tarjeta:** nombre `PR UE 01 Toyota Yaris · CITA 30-09-2026 16:30` (o el nombre del cliente sin vehículo), fecha de vencimiento en hora de Chile (`marcaTiempoChile`, verificada en verano UTC-3 e invierno UTC-4), descripción con cliente, teléfono, isla, servicio y origen, y los campos Patente, Datos del cliente, Tipo de servicio y Observaciones.
- **Cambios:** si cambia fecha, hora o servicio, se actualiza la tarjeta. Si la cita se cancela o el cliente no asiste, la tarjeta se borra SOLO si sigue en "agenda" y ninguna OT la adoptó; si alguien ya la movió, no se toca. Una tarjeta que ya no existe en ClickUp solo limpia el vínculo. Una cita "completada" (la OT ya nació) no toca nada.
- **La OT adopta la tarjeta:** `clickup-sincronizar` (v14), si la OT tiene `cita_id` y esa cita ya tiene tarjeta, la renombra al formato de la OT y la vincula en vez de crear otra. Verificado: OT 5039 y su cita quedaron con la misma tarjeta.
- **Campos personalizados por empresa:** `clickup_config.campos` (jsonb) guarda los IDs de los campos de la lista de cada empresa; null = los de la lista real (fijos en `_shared/clickup.ts`). `scripts/clickup-demo.mjs verificar` compara los campos de ambas listas por nombre e imprime el SQL si difieren. Hallazgo: la lista de prueba solo tiene "Inspección Completada"; hay que crear los 6 campos en ClickUp (la API no permite crearlos). Mientras tanto la tarjeta se crea igual con toda la información en la descripción y los campos registran errores esperados ("Custom field does not exist in the task location hierarchy").
- **Agenda** (`Agenda.jsx`): al guardar una cita y al abrirse llama a `clickup-agendar-cita` (nunca bloquea la pantalla; si falla, avisa y reintenta al abrir); las citas ya enviadas muestran "en ClickUp".
- **Bug encontrado y corregido probando:** `AuthContext` trataba cada evento `SIGNED_IN`/`TOKEN_REFRESHED` de Supabase -que se emite cada vez que la pestaña recupera el foco- como un inicio de sesión nuevo: marcaba "Cargando…" y desmontaba la pantalla, perdiendo lo que la persona estaba escribiendo (un formulario a medias, la fecha elegida en la Agenda). Ahora, si es la misma persona con el perfil ya cargado, no se recarga nada.
- **Script:** `clickup-demo.mjs` comparaba el ID de lista de un webhook como texto, pero ClickUp lo devuelve como número, así que decía "falta el webhook" aunque existía; corregido.
- **Probado de punta a punta en la demo:** cita nueva crea la tarjeta en "agenda" (confirmado en ClickUp por la persona), cancelarla la borra (`retiradas: 1`), una OT creada desde la cita adopta la tarjeta, y procesar la cita completada no borra nada. **No probado:** el camino del bot de WhatsApp (sin número conectado; usa el mismo código compartido) ni la reprogramación (la Agenda no tiene edición de fecha; el trigger la cubre).
